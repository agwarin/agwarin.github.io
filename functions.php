...
//require_once( $template_directory . '/inc/wp-updates-theme.php');
//new WPUpdatesThemeUpdater_483( 'http://wp-updates.com/api/2/theme', basename( $template_directory ) );
...
function altitude_widgets_init() {
	...
	register_sidebar( array(
		'name'          => __( 'Home Widgets', 'altitude' ),
		'id'            => 'home-widgets',
		'description'   => __( 'Home Page Bottom Widgets Area', 'altitude' ),
...
function altitude_scripts() {
	...
	wp_enqueue_script( 'altitude-theme', $template_directory_uri . '/js/theme-js.js', array( 'jquery', 'altitude-stellar' ), '20130825', true );

	// Add Sanchez google font, used in the main stylesheet.
	wp_enqueue_style( 'altitude-sanchez' );
...
/**
 * Used to display the contact details saved in theme_mods
 * change in customizer
 *
 * @param $type string phone, address or email
 *
 * @echos string
 */
function altitude_contact_details( $type ) {

	switch( $type ) {
		case 'phone':
			$contact = get_theme_mod( 'altitude_phone_number' );
			$render  = '<div class="contact-item">' . esc_html( $contact ) . '</div>';
			break;
		case 'address':
			$contact = get_theme_mod( 'altitude_address' );
			$render  = '<div class="altitude-item">' . esc_html( $contact ) . '</div>';
			break;
		case 'email':
			$email   = get_theme_mod( 'altitude_email' );
			$contact = '<a href="mailto:' . antispambot( sanitize_email( $email ) ) . '" title="' . __( 'email address', 'altitude' ) . '">' . esc_html( $email ) . '</a>';
			$render  = '<div class="altitude-item">' . $contact . '</div>';
			break;
		default:
			$render = '';
	}
